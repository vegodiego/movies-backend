...
const requireApiUser = async (req, res, next) => {
  try{
    const token = req.header("Authorization");  //token enviado en el encabezado
  	const decoded = jwt.verify(token, process.env.SECRET_KEY); //decodificación del token para ver si es válido (si si, se tiene el objeto {userId: user._id} guaradado en el post login)
  	const user = await User.findById(decoded.userId); //buscando el userId de la decodificación en el modelo User
  	if (user){ //token válido
  		next();
  	}
